...
#Each object is of the class `r class(pl)` which is a [S4 class](http://adv-r.had.co.nz/S4.html). The class will influence how we will #interact with them. TL;DR S4 is one of the most popular OOP programming R #type.
#It is easy to recognize that S4 attributes are  by usage of `@` for #accessing an instance methods and attributes.
...
It simply evaluate the provided code for certain plot/plots and save it as png file.
We could reuse these visualizations e.g. on the web page as a static files. Moreover in Mac OS RStudio (my environment not precisely yours) the graphics system seems to be extremely inefficient. I even commented the already exists issue on [github](https://github.com/rstudio/rstudio/issues/3866).
Later ggplot objects will be saved with `ggsave` function.

```{r}
plot_2png <- function(x, filename, printed = FALSE) {
  x <- substitute(x)
  stopifnot(is.call(x))
  ...
  if (printed) {
    if (Sys.info()["sysname"] != "Windows") {
      system(paste0('open "', filename_path, '"'))
    } else {
      system(paste0('start "', filename_path, '"'))
    }
  }
}
```
...
CRS before transforation and the box dimensions.
...
dispatch
https://adv-r.hadley.nz/s4.html#s4-dispatch
...
```{r, message = FALSE}
ggploty <- ggplot(data.frame(raster::geom(pl)), aes(x = x, y = y)) +
  geom_polygon()
...
Number of coordinates (centroids) will be the same as number of shapes (like districts).
That is what we have only one coordinate for `pl`.
...
```{r}
library(classInt)
unemploy <- data_nts$XA21[data_nts$year == 2015]
summary(unemploy)
```


We want to create a plot with certain color assign for each interval. Moreover the proper legend is created.
...
# when e.g. style = "knn" then we get intervals automatically
...
```{r}
library(GISTools)
pov_dists <- data_nts$dist[data_nts$year == 2015]
summary(pov_dists)
...
Extra tasks
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Plot a contour map of average salary (Poland=100). Label interactively the mostly outlying regions. 
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
choropleth(pov[1:10,], salary[1:10])
identify(crds_pov[1:10,], labels = pov_nams[which_top][1:10], cex=0.6)
```

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Plot poviat map for distances between poviats and regional core cities – use palette of grey colours. Erease administrative contours using “transparent” colour. Label the regional core cities. 




~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~Draw a map of average salary in a selected voivodship, other than Lubelskie. Apply poviat name labels using the text () command.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For point (Lubelskie voivodship) data plot a map of distance between randomly selected 5000 points and powiat cities: Biała Podlaska, Zamość, Chełm, Lublin. Connect the points with the nearest cities. Add poviat countours. Use spDistsN1() command. 


```r
frims$runif <- runif(dim(firms)[1], 0, 1)
sub <- sample(dim(firms)[1], 5000, replace = FALSE)
firms.sub <- firms[sub, ]
zamość <- c(23.2134931, 50.7213773)
chelm <- c(23.4196655, 51.1355623)
lublin <- c(22.4236853, 51.2180254)

dist.zamość <- spDistsN1(as.matrix(firms.sub[, 12:13]), zamość, lonlat = TRUE)
dist.lublin <- spDistN1(as.matrix(firms.sub[, 12:13]), lublin, lonlat = TRUE)
```


Let’s use over(). Below you have the structure of the code, which needs some corrections. The goal is to plot in red the points, which belong to selected municipality (gmina). 
